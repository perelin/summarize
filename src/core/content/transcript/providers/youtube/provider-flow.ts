...
import { fetchSubtitlesWithYtDlp } from "./yt-dlp-subs.js";
...
export async function trySubtitleTranscript(
  flow: YouTubeProviderFlow,
  skipAutoGenerated = false,
): Promise<ProviderResult | null> {
  if (!flow.options.ytDlpPath) return null;

  flow.pushHint("YouTube: captions unavailable in page; checking yt-dlp subtitles");
  flow.attemptedProviders.push("yt-dlp-subs");

  const result = await fetchSubtitlesWithYtDlp({
  ...
  return {
    text: normalizeTranscriptText(result.payload.text),
    source: "yt-dlp-subs",
    segments: flow.options.transcriptTimestamps ? (result.payload.segments ?? null) : null,
    metadata: {
      provider: "yt-dlp-subs",
      subtitleKind: result.kind,
