...
export class SseSessionManager {
  ...
  /** Push an event into a session's buffer. Silently no-ops if the session is missing or the buffer cap is exceeded. */
  pushEvent(sessionId: string, event: SseEvent): void {
    const session = this.sessions.get(sessionId);
    if (!session) return;

    // Check if already over cap — stop buffering
    if (session.totalBytes >= BUFFER_CAP_BYTES) return;
  ...
  /** Destroy a session explicitly. Silently no-ops if the session doesn't exist. */
